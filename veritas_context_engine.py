...
ACCOUNTABILITY_CRISIS_KEYWORDS = [
    ...
    # Ukrainian accountability context UK
    r'\b(набу|нап|антикорупційн|детектив|підозра|обшук|затримання|арешт)\b',
    r'\b(розслідування|корупція|хабар|розтрата|зловживання|відмивання)\b',
    r'\b(відставка|звільнення|відсторонення|імпічмент|відповідальність)\b',
    r'\b(витік|злив|схема|офшор|декларація|незаконне збагачення)\b',
]
...
def detect_text_topic(text: str, genre: Optional[str] = None) -> Optional[str]:
    """
    Detect if the text clearly belongs to a non-political domain.
    Returns topic name (e.g. 'SPORT') or None if displacement analysis should proceed.

    Priority:
    1. If genre is provided by GenreDetector — trust it completely.
       Only block displacement for non-editorial genres.
    2. Fallback: crude regex (requires 3+ hits to reduce false positives).

    Genres that BLOCK displacement analysis (not news/political):
      SPORT, CULTURE, LIFESTYLE, SATIRE

    Genres that ALLOW (and amplify) displacement analysis:
      CONSPIRACY_NEWS — this is exactly what displacement is designed to catch
      ANALYTICS, REPORT, OPINION, UNKNOWN — proceed normally
      SCIENCE, TECH — proceed (science news can be used as displacement)
    """
    # ── Priority 1: trust GenreDetector result ────────────────────────
    NON_EDITORIAL_GENRES = {'SPORT', 'CULTURE', 'LIFESTYLE', 'SATIRE'}
    if genre:
        if genre in NON_EDITORIAL_GENRES:
            return genre   # block displacement
        else:
            return None    # proceed — especially for CONSPIRACY_NEWS

    # ── Priority 2: fallback regex (raised threshold to 3 hits) ──────
    # Prevents "score", "match", "coach" from firing on news articles.
    ...
    for topic, patterns in NON_NEWS_TOPIC_PATTERNS.items():
        ...
        if hits >= 3:   # raised from 2 → 3 to reduce false positives
            scores[topic] = hits
    if scores:
        best = max(scores, key=scores.get)
        # Even with 3+ hits, never block SCIENCE or TECH —
        # scientific/tech framing is often used in news displacement
        if best in ('SCIENCE', 'TECH'):
            return None
        return best
    return None
...
class ContextEngine:
    ...
    def analyze_displacement(self, text: str, genre: Optional[str] = None) -> Dict:
        """
        Core method: given a text, check if it looks like a displacement
        relative to the current information field.

        Args:
            text:  the article/post text to analyze
            genre: genre from GenreDetector (optional but preferred).
                   When provided, takes priority over internal topic detection.
                   CONSPIRACY_NEWS always bypasses the topic guard.
        ...
        # ── Topic Guard: блокуємо displacement тільки для нередакційних жанрів.
        # genre з GenreDetector має пріоритет над regex — щоб уникнути хибного
        # спрацювання на загальних словах типу "score", "match", "coach".
        # CONSPIRACY_NEWS завжди проходить далі — це саме те що displacement ловить.
        detected_topic = detect_text_topic(text, genre=genre)
        if detected_topic:
            topic_labels_uk = {
                'SPORT':     'спортивний',
                'CULTURE':   'культурний',
