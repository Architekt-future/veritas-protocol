...
class ManipulationDetector:
    ...
    def __init__(self):
        ...
        self.pattern_sets = [
            ...
            # Анонімна "більшість" як авторитет без джерел
            {
                'name': 'MANUFACTURED_CONSENSUS',
                'score': 0.40,
                'min_hits': 2,
                'patterns': [
                    r'(більшість (досліджень|експертів|аналітиків|вчених)).{1,60}(свідч|підтверджу|вказу|показу)',
                    r'(дедалі більше (аналітиків|дослідників|експертів)).{1,60}(сходяться|вважають|визнають)',
                    r'(сходяться на думці|прийшли до висновку|погоджуються).{1,40}(більшість|провідні|світові)',
                    r'(загальновизнано|загальноприйнято|не викликає сумнівів).{1,60}(що|як)',
                    r'(деякі (дослідники|аналітики|експерти)).{1,60}(оцінюють|підрахували|встановили).{1,60}(\\d)',
                    r'(досвід (показує|свідчить|доводить)).{1,40}(що|як).{1,60}(завжди|зазвичай|як правило)',
                    r'(як (відомо|показує практика|демонструє досвід)).{1,60}без.{1,40}(джерела|посилання)',
                ],
            },
        ]
