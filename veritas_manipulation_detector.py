...
class ManipulationDetector:
        ...
        self.pattern_sets = [
            ...
            # Attacks reader's ability to perceive / reason
            {
                ...
                'patterns': [
                    ...
                    r'(ваш|ваша|ваше|ваші|твій|твоя).{1,40}(обмежен).{1,60}(сприйняття|розуміння|свідомість|кругозір)',
                    r'(ваш|ваша|ваше|ваші|твій|твоя).{1,40}(когнітивн|ментальн).{1,60}(деградац|обмежен|дефіцит|блокуван)',
                    ...
                    r'(your).{1,40}(limited|narrow).{1,60}(perception|understanding|consciousness|perspective)',
                    r'(your).{1,40}(cognitive|mental).{1,60}(degradation|limitation|deficit|blockage)',
            ...
            # monopoly_argument в synthesis/oracle) для повного покриття.
            {
                ...
                'patterns': [
                    ...
                    # v2 (14.09.2026): "лише" розширено на "тільки" (частотніший
                    # синонім), дієслівна група розширена з однини (може/здатна/
                    # гідна) на множину і "маємо/мають" — виявлено емпірично на
                    # смоук-тесті синтетичних Mythos-текстів (4 бренди), де regex
                    # 0/4 при живому патерні "Тільки ми маємо..."/"Тільки
                    # організації нашого масштабу можуть..."
                    r'(лише|тільки) (ми|наша команда|організаці[ії]|організація з нашим).{1,200}(може|можут\w*|здатн\w*|гідн\w*|маємо|мають)',
                    ...
                    # v2: "не привілей, а необхідність" — окремий патерн (не
                    # прив'язаний до "концентрація...в руках", бо в реальних
                    # текстах ці два блоки часто рознесені на абзац). Корінь
                    # "привіле\w*", не "привілей\w*" — покриває непрямі
                    # відмінки (привілеєм, привілею) з чергуванням й→є.
                    r'не\s+привіле\w*,?\s*а\s+необхідніст\w*',
                    r'необхідніст\w*,?\s*а\s+не\s+привіле\w*',
                    # v2: "якщо не ми, то хто" — найхарактерніший маркер
                    # preemptive monopoly, раніше взагалі не мав покриття.
                    r'якщо не (ми|наша команда|ми зробимо).{1,60}то.{0,20}хто.{0,40}(гарантує\w*|гарантувати|зможе|спромож\w*|здатн\w*|зупинить\w*|врятує\w*|захистить\w*|забезпечить\w*)',
                    ...
                    # v2 (14.09.2026): EN-дзеркало тих самих трьох виправлень
                    r'(only|solely) (we|our team|an organization with our).{1,200}(can|is able to|is positioned to|has)',
                    r'not a privilege,?\s*but\s+a\s+necessity',
                    r'a\s+necessity,?\s*not\s+a\s+privilege',
                    r'if not (us|we).{1,60}(then\s+)?who.{0,40}(guarantee\w*|can (ensure|prevent|stop|safeguard)|is (able|positioned) to|capable of)',
],
            },
            ...
            # System designed to be unfalsifiable / immune to critique
            {
                ...
                'patterns': [
                    ...
                    # ФІКС v20.6.3: 'без доказів' видалено — це звичайний
                    # журналістський зворот для опису БУДЬ-ЯКОГО
                    # непідтвердженого твердження ("звинувачення без
                    # доказів"), смислово ПРОТИЛЕЖНИЙ до "докази не
                    # потрібні" (епістемічне замикання — переконання, що
                    # свідомо відмовляється від обґрунтування). Лишились
                    # тільки патерни, що явно стверджують непотрібність
                    # доказів для віри, а не констатують їх відсутність.
                    r'(докази.{1,40}не потрібн|доказів не потребує)',
                    ...
                    # 'beyond evidence' видалено — та сама причина, що й
                    # 'без доказів' вище: описує звичайне непідтверджене
                    # твердження, не епістемічне замикання.
                    r'(evidence.{1,40}not needed|no proof required)',
            ...
            # Анонімна "більшість" або "всі" як авторитет без джерел
            {
                ...
                'patterns': [
                    ...
                    r'(безсумнівно що|безперечно що)',
            ...
            # Провина як зброя: ваша автономія = борг, думати = злочин
            {
                ...
                'score': 0.55,
                # min_hits підняли з 1 до 2 (превентивно, за аналогією з
                # PSEUDO_OBJECTIVE_FALLACY): патерни "інформаційний/когнітивний
                # привілей" та "критика...привілей" — самодостатні академічні
                # терміни (privilege theory), не самі по собі маркер
                # маніпуляції. Справжнє guilt weaponization — коли такий
                # термін поєднується з іншим маркером цієї ж категорії.
                'min_hits': 2,
                'patterns': [
            ...
            # Науковий стиль для приховування бездоказових тверджень
            {
                ...
                'score': 0.50,
                # min_hits підняли з 1 до 2: перший патерн ("з точки зору X
                # аналізу") — нейтральний академічний зворот, не сам по собі
                # епістемічне закриття. Реальна маніпуляція в цій категорії —
                # коли він поєднується з іншим маркером закриття
                # ("не є питанням думки", "не можуть бути ефективними" тощо).
                'min_hits': 2,
                'patterns': [
            ...
            # "У нас так прийнято", "ми всі знаємо" — апеляція до невизначеної спільноти
            {
                ...
                'min_hits': 1,
                'patterns': [
                    # ФІКС v20.6.2: раніше `(у нашій (X) прийнято|заведено|вважається)` —
                    # через відсутність внутрішнього групування `|` ділив увесь
                    # вираз на ТРИ незалежні альтернативи: повну фразу "у нашій
                    # X прийнято", і окремо самостійні слова "заведено" та
                    # "вважається" — які матчились БУДЬ-ДЕ в тексті незалежно від
                    # контексту. "вважається" — одне з найзвичайніших слів
                    # укр. мови (пасивна конструкція "X вважається Y"), тому
                    # спрацьовувало майже на кожному тексті. Виправлено
                    # групуванням: дієслово тепер частина тієї самої фрази.
            ...
            # ── VEILED THREAT ─────────────────────────────────────────────
            # (14.09.2026, додано за одним реальним прикладом — заява МЗС
            # Польщі щодо вшанування Україною нацгероїв — але узагальнено за
            # відомою структурою прийому з дипломатичної/пропагандистської
            # риторики, а не виведено виключно з цього одного речення.
            #
            # Структура ("velvet-glove threat" / conditional threat):
            #   [визнання чужого права/автономії] + [зворотний сполучник] +
            #   [невизначена, безсуб'єктна загроза наслідків]
            # Сила прийому саме в тому, що погроза ніде не сформульована
            # прямо (немає названого суб'єкта дії чи конкретного наслідку) —
            # формальна ввічливість/нейтральність робить її важчою для
            # прямої фактчекінгової фальсифікації, ніж відкриту погрозу.
            #
            # Ризик хибних спрацювань контролюється тим, що вимагається
            # ЗБІГ УСІХ ТРЬОХ компонентів у вузькому вікні — саме собою
            # "але" чи "обережність" ніде не тригерить.
            {
                'name': 'VEILED_THREAT',
                'score': 0.55,
                'min_hits': 1,
                'patterns': [
                    r'(власна справа|власний вибір|ваше право|це ваш вибір|ніхто не заперечує.{1,20}права).{1,80}(але|проте|однак|тим не менш|водночас).{1,80}(обережн\w*|не.{0,15}дивуватис[ья].{1,20}наслідк\w*|готуйтесь? до наслідк\w*|наслідки (будуть|можуть|не забаряться)|доведеться (пожалкувати|відповідати|платити))',
                    # самодостатні ідіоми — настільки специфічні, що не
                    # потребують сусіднього блоку "визнання права", ризик
                    # хибного спрацювання низький через саму рідкісність фрази
                    r'(не кажіть,? що (вас|ми)( вас)? не попереджали)',
                    r'(потім не дивуйтеся|потім не дивуватися)',
                    r'(час покаже,?\s*чи.{1,30}було\s+(мудрим|правильним|виваженим))',
                
                    r'(your (own )?(business|right|choice)|no one (disputes|questions) your right).{1,80}(but|however|that said|nevertheless).{1,80}(be careful|caution|should\w*n?\'?t be surprised.{1,20}consequences?|consequences? (will|may|are coming)|will (regret|have to answer|pay))',
                    r'(don\'t say (you|we) (weren\'t|werent) warned)',
                    r'(time will tell (if|whether) (that|this) was (wise|the right (call|decision)))',
],
            },
        {
    ...
    def analyze(self, text: str) -> Dict:
        ...
        # в preemptive_monopoly_slots.py — щільність метамовних маркерів
        # (текст ПРО прийом, а не ІНСТАНЦІЯ прийому). Незалежний від
        # бектик-фіксу вище сигнал: спрацьовує навіть якщо приклад
        # процитовано без бектиків/лапок узагалі (переказ своїми словами).
        # НЕ вайтліст — м'яке зниження ваги, той самий принцип, що й
        # ATTRIBUTION_SHIELD: сигнал не зникає, лише синус довіри до нього
        # нижчий, коли контекст явно документаційний.
        _META_MARKERS = (
            r'риторичн\w*|прийом\w*|патерн\w*|маркер\w*|конструкці\w*|'
            r'формулюванн\w*|цитат\w*|приклад\w*|детектор\w*|слот\w*|'
            r'ілюстраці\w*|наприклад|\bregex\b|\bscorer\b'
        )
        meta_count = len(set(re.findall(_META_MARKERS, text_lower, re.IGNORECASE)))
        if meta_count >= 3 and manipulation_score > 0:
            manipulation_score = round(manipulation_score * 0.15, 3)
