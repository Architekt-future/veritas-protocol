...
class ARDChecker:
    ...
    INEVITABILITY_ABSOLUTION_UK = [
        ...
        # ФІКС v20.6.3: попередня версія цього патерну спиралась ЛИШЕ на
        # "на жаль" + відсутність заклику до дії поруч — і цього виявилось
        # замало: "на жаль" саме по собі є найзвичайнісіньким виразом
        # прикрості/жалю про БУДЬ-ЩО (напр. "На жаль, його вже немає серед
        # нас" — просте співчуття про смерть героя, geть не "неминучість
        # як відпущення"). На відміну від сусідніх патернів у цьому списку
        # (які всі вимагають конкретної фаталістичної конструкції —
        # "нічого не вдієш... такий світ", "завжди так було... сила"),
        # цей був єдиним без вимоги до самої фаталістичної семантики.
        # Тепер вимагає явної фрази неможливості/невідворотності поруч —
        # той самий рівень специфічності, що й решта списку.
        r'(на жаль|на превеликий жаль).{1,60}(нічого (не можна|не вдієш|не зміниш|не поробиш)|'
        r'неможливо (щось )?змінити|цього (не змінити|не уникнути)|це неминуче|'
        r'ніхто (нічого )?не (може|здатен|в змозі))',
    ...
    # v20.6.2: окремий guard для принципу IA (Неминучість як відпущення).
    # Причиновий guard вище перевіряє "чи пояснена причина" — інше
    # питання, ніж "чи є заклик до дії поруч". Текст типу "На жаль, X не
    # буде. Над цим потрібно буде важко працювати" — це НЕ звільнення від
    # відповідальності (де фаталізм замінює дію), а визнання складних
    # обставин РАЗОМ із закликом до зусиль — протилежний паттерн. Раніше
    # це намагались зловити одним регексом з (?!...) одразу після
    # квантифікатора — технічно не працює для {1,N} (задовольняється вже
    # при meньшій кількості повторень, не вимагає дійти до кінця
    # діапазону). Правильне рішення — окрема перевірка в коді, як і для
    # причинового guard.
    _CALL_TO_ACTION_MARKERS = re.compile(
        r'(треба|слід|необхідно|потрібно|маємо|варто|повинн\w+|'
        r'must|need to|have to|should|ought to)',
        re.IGNORECASE
    )

    def _has_call_to_action_nearby(self, text_lower: str, start: int, end: int,
                                     window: int = 150) -> bool:
        lo = max(0, start - window)
        hi = min(len(text_lower), end + window)
