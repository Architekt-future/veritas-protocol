...
class ARDChecker:
    ...
    PRINCIPLE_IX_EN = [
        r'(formally|technically|legally).{1,60}(not (violating|prohibited|a crime)).{1,60}(but)',
        r'(de jure|on paper|officially).{1,60}(everything is (clean|correct|legal)).{1,60}(but|however)',
        r'(found (a way|a loophole|a workaround)).{1,60}(to (avoid|circumvent|not violate))',
        r'(not violating the (letter|rule|law)).{1,60}(but (the spirit|the intent|the essence))',
        r'(everything is (legal|lawful|official)).{1,60}(although|even though|despite).{1,40}(questionable|dubious|dishonest)',
    ]
