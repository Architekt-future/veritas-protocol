...
class ARDChecker:
    ...
    INEVITABILITY_ABSOLUTION_UK = [
        ...
        # ФІКС v20.6.2: (?!X) ПІСЛЯ .{1,80} перевіряв лише ОДНУ позицію
        # (куди дожадібно доїхав квантифікатор), а не "чи є X десь у цих
        # 80 символах". Правильна конструкція — повторюваний lookahead на
        # кожній позиції: (?:(?!X).){1,80}. Заразом розширено словник
        # заклику до дії: раніше вимагалось "але треба/слід/маємо/варто/
        # необхідно" — union="але"+дієслово одразу поруч; але "потрібно"
        # (синонім "треба") був відсутній, і сама вимога "але" одразу
        # перед дієсловом виключала звичайне окреме речення-заклик без
        # сполучника (напр. "На жаль, X не буде. Над цим потрібно
        # важко працювати." — тут заклик є, просто без "але").
        r'(на жаль|на превеликий жаль)(?:(?!треба|слід|необхідно|потрібно|маємо|варто).){1,80}',
