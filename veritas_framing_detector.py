...
class VeritasFramingDetector:
    ...
    GROUND_PREP_EN = [
        ...
        r'\b(growing|increasing|emerging)\s+(debate|discussion|questions?|concern|doubt)\b',
        r'\b(some|many)\s+(experts?|analysts?|observers?)\s+(are\s+)?(starting\s+to|beginning\s+to|now)\s+(question|doubt|ask)\b',
        r'\b(more\s+and\s+more|increasingly)\s+(people|voices?|experts?)\s+(are|have\s+started)\s+(questioning|doubting|asking)\b',
        r'\bthe\s+(question|debate)\s+(is\s+)?(now|increasingly|no\s+longer)\b',
    ]
    ...
    OVERTON_EN = [
        ...
        r'\b(what\s+was\s+once|previously|formerly)\s+(unthinkable|taboo|impossible|out\s+of\s+the\s+question)\b',
        r'\b(now\s+openly|openly\s+now|increasingly)\s+(discussed|debated|considered|on\s+the\s+table)\b',
        r'\b(previously|once|formerly)\s+(taboo|unthinkable|radical)\s+(idea|option|proposal)\b',
    ]
    ...
    PRESUP_EN = [
        ...
        r'\bwhen\s+did\s+\w[\w\s]{3,60}(stop|become|start|lose)\b',
        r'\bhow\s+long\s+(before|until)\s+\w[\w\s]{3,60}(admits?|acknowledges?|realizes?)\b',
        r'\bwhy\s+(does|do|is|are)\s+\w[\w\s]{3,50}(still|continue\s+to|keep)\b',
    ]
    ...
    JUXTAPOSITION_EN = [
        ...
        r'\bwhile\s+\w[\w\s]{3,50}(die|suffer|struggle|sacrifice)\b.{0,80}\b(politicians?|elites?|officials?)\b',
        r'\b(heroes?|soldiers?|workers?)\s+(sacrifice|give|risk).{0,60}\b(elites?|politicians?)\s+(take|enjoy|profit)\b',
        r'\bwhereas\s+\w[\w\s]{3,50}(ordinary|common|regular)\s+(people|citizens?)\b',
    ]
    # ── Pattern 7: Enumerated Inevitability ─────────────────────────────────
    ...
    WEIGHTS = {
        ...
        'presupposition':           0.20,
        'juxtaposition':            0.25,
        'enumerated_inevitability': 0.28,
        'scenario_as_fact':         0.25,
    }
    ...
    def analyze(self, text: str) -> FramingResult:
        ...
        hits = {
            ...
            'enumerated_inevitability': self._count(t, self.ENUMERATED_INEVITABILITY_UK + self.ENUMERATED_INEVITABILITY_EN),
            'scenario_as_fact':   self._count(t, self.SCENARIO_AS_FACT_UK + self.SCENARIO_AS_FACT_EN),
        }
