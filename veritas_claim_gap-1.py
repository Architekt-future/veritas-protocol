...
class ClaimGapDetector:
    ...
    CLAIM_MARKERS_EN = [
        ...
        # War/crisis predictions EN
        (r'\b(end of the war|war will end|peace is coming|ceasefire is near)\b', 0.80),
        (r'\b(victory (is|will be) (near|close|imminent|certain))\b', 0.75),
        (r'\b(gave (a )?forecast|made (a )?prediction|issued (a )?warning)\b', 0.65),
        (r'\b(experts? (predict|forecast|foresee)).{0,40}(will|is going to)\b', 0.65),
        # Revelation framing EN
        (r'\b(the truth (about|behind)|the real reason|secret revealed)\b', 0.80),
        (r'\b(finally (revealed|exposed|confirmed|proven))\b', 0.70),
        (r'\b(what (they|no one) (didnt|dont) want you to know)\b', 0.85),
    ]
