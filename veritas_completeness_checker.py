...
class ContextCompletenessChecker:
    ...
    def analyze(self, text: str) -> Dict:
        ...
        # Flag missing consequences only for action-oriented texts
        ...
        # Flag missing counterposition only for assertive texts
        ...
        # Flag missing source only if it looks like a factual claim
        ...
        if completeness_score >= 0.75:
            ...
            note = (
                'Текст містить значущі твердження, але відсутні кілька '
                'контекстуальних вимірів. Свідок рекомендує перехресну перевірку.'
            )
        elif completeness_score >= 0.50:
            verdict = 'ЧАСТКОВИЙ_КОНТЕКСТ'
            note = (
                'Деякі виміри контексту відсутні. '
                'Для повної картини варто звернутись до додаткових джерел.'
            )
        ...
        return {
            ...
            'is_advisory_only': True,  # Never blocks — only informs
        }
