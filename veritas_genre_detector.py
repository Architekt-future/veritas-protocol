...
class GenreDetector:
    ...
    INTERVIEW_SIGNALS = [
        ...
        # v20.6: типова укр. конвенція інтерв'ю без явних Q:/A: міток —
        # атрибуційний рядок формату "Редактор/кореспондент X ставить
        # запитання Y" (замінник em-dash чи "Q:"-розмітки, дуже поширений
        # у Ukrainian-мовних медіа). Раніше жоден патерн це не ловив, тому
        # довгі якісні інтерв'ю (напр. з археологами, науковцями) системно
        # класифікувались як UNKNOWN.
        r'\b(ставить\s+запитання|поставив\s+запитання|задав\s+запитання)\b',
        r'\b(у\s+відповідь\s+на\s+запитання|відповідаючи\s+на\s+запитання)\b',
    ]
    ...
    GEOPOLITICS_SIGNALS = [
        r'\b(nato|нато)\b',
        r'\b(sanctions?|санкці\w*)\b',
        r'\b(diplomatic|дипломатичн\w*)\b',
        r'\b(sovereignty|суверенітет\w*)\b',
        r'\b(ceasefire|перемир.я)\b',
        r'\b(kremlin|кремл\w*)\b',
        r'\b(geopolit\w*|геополіт\w*)\b',
        ...
        r'\b(annexation|анексі\w*)\b',
        r'\b(occupation|окупац\w*)\b',
    ...
    ECONOMY_SIGNALS = [
        ...
        r'\b(unemployment|безробітт\w*)\b',
