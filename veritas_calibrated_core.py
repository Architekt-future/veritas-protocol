...
try:
    from veritas_genre_detector import GenreDetector
    GENRE_DETECTOR_AVAILABLE = True
except ImportError:
...
try:
    from veritas_laundered_claim import LaunderedClaimDetector
except ImportError:
    LaunderedClaimDetector = None
...
class VeritasCalibratedCore:
    ...
    def __init__(self):
        ...
        self.laundered_claim_detector = LaunderedClaimDetector() if LaunderedClaimDetector else None
    ...
    def analyze(self, text: str) -> Dict:
        ...
        # ---- PHASE 0.5: GENRE DETECTOR ----
        if self.genre_detector:
            _genre_result = self.genre_detector.analyze(text)
            _genre = _genre_result.genre
            _genre_calibration = _genre_result.calibration
        else:
            _genre = 'UNKNOWN'
            _genre_calibration = {'absurdity_weight': 1.8, 'anon_authority': True,
                                   'unanchored_claim': True, 'entropy_damper': True,
                                   'entropy_cap': 1.0}
        print(f'🎭 GENRE: {_genre}')
        _genre_entropy_cap = _genre_calibration.get('entropy_cap', 1.0)
        _genre_floor_boost = 0.15 if _genre == 'ANALYTICS' else 0.0
        ...
        # Apply genre cap: analytics text should never be penalized for high entropy
        shannon_entropy = min(shannon_entropy, _genre_entropy_cap)
        ...
        # Skip for genres where financial framing is not the primary concern.
        ...
        if self.lac_finance and _genre not in LAC_FINANCE_SKIP_GENRES:
        ...
        if self.pseudoscience_detector and _genre not in PSEUDOSCIENCE_SKIP_GENRES:
        ...
        # ---- PHASE: LAUNDERED CLAIM DETECTOR v1.0 ----
        laundered_result = {
        ...
        if _genre == 'ANALYTICS':
            # Analytics: don't apply entropy damper — high entropy is expected
            adjusted_entropy = min(shannon_entropy, 0.75)
        else:
        ...
        # For ANALYTICS genre: suppress ANONYMOUS_AUTHORITY — it's expected in multi-source journalism
        if _genre == 'ANALYTICS':
            lac_ii_violations = [v for v in lac_ii_violations 
                                  if v.vtype not in ('ANONYMOUS_AUTHORITY', 'UNANCHORED_CLAIM')]
        ...
        if is_protected_science:
            ...
            # For ANALYTICS genre: reduce absurdity weight (geopolitical metaphors are normal)
            _absurdity_weight = 0.5 if _genre == 'ANALYTICS' else 1.8
            if absurdity_result['absurdity_score'] > 0:
                base_score += absurdity_result['absurdity_score'] * _absurdity_weight
        ...
        # AXIOM GUARD OVERRIDE
            ...
            if (_genre == 'ANALYTICS' and
                    lac_finance_result.get('is_financial') and
            ...
            # CONSPIRACY_NEWS gets precise implied-causality verdict
            if _genre == 'CONSPIRACY_NEWS':
                status, verdict = 'WARNING', 'ІМПЛІКОВАНА ПРИЧИННІСТЬ'
                explanation = 'Текст будує оповідь навколо збігу в часі — дві події відбулись поруч, і читач сам домальовує зв\'язок між ними. Це не обов\'язково брехня: обидва факти можуть бути реальними. Але «після» не означає «через» — перевірте чи є пряме підтвердження зв\'язку між подіями.'
            else:
                status, verdict = 'WARNING', 'АБСТРАКТНА СКЛАДНІСТЬ'
                explanation = 'Текст оперує загальними поняттями без конкретних фактів або прикладів. Це не обов\'язково маніпуляція, але перевірити такі твердження складно. Шукайте конкретні цифри, джерела та факти перш ніж довіряти.'
        elif final_score > 0.15:
            # v14.3: Diplomat label (was "ПРИЙНЯТНА ІНФОРМАЦІЯ")
            # CONSPIRACY_NEWS with low score = still note the framing
            if _genre == 'CONSPIRACY_NEWS':
                status, verdict = 'ACCEPTABLE', 'ІМПЛІКОВАНА ПРИЧИННІСТЬ'
                explanation = 'Факти у тексті виглядають реальними, але подані поруч без доведеного зв\'язку. Це журналістський прийом «збігу» — читач самостійно робить висновок якого автор уникає. Перевірте кожен факт окремо.'
            else:
                status, verdict = 'ACCEPTABLE', 'ВЕРИФІКОВАНА ЛОГІКА'
                explanation = 'Текст має чітку структуру і твердження логічно пов\'язані між собою. Ознак маніпуляції не виявлено, але це не замінює перевірку фактів у незалежних джерелах.'
        else:
            if _genre == 'ANALYTICS':
                # If LAC Finance detected IMITATION OF LOGIC — don't reward with clean analytics verdict
                _lac_finance_imitation = (
                    lac_finance_result.get('is_financial') and
                    lac_finance_result.get('score', 1.0) <= 0.25
                )
                if _lac_finance_imitation:
                    status, verdict = 'WARNING', 'СТРУКТУРОВАНА РИТОРИКА'
                    explanation = 'Текст використовує аналітичні формулювання, але уникає конкретики: немає чітких трейдофів, відповідальних осіб чи критеріїв успіху. Це може бути корпоративна або політична риторика замаскована під аналітику. Запитайте себе: хто саме відповідає за ці слова і що буде якщо нічого не зміниться?'
                else:
                    status, verdict = 'VERIFIED', 'АНАЛІТИЧНА СТРУКТУРОВАНІСТЬ'
                    explanation = 'Текст побудований як аналітичний матеріал: посилається на різні джерела, використовує обережні формулювання. Це ознаки журналістської чесності, а не маніпуляції. Все одно варто перевірити ключові факти самостійно.'
            elif _genre == 'OPINION':
                status, verdict = 'VERIFIED', 'АВТОРСЬКА ПОЗИЦІЯ'
                explanation = 'Це текст де автор висловлює власну думку, а не повідомляє факти. Такий формат є нормальним, але важливо пам\'ятати: думка однієї людини — не доказ. Оцінюйте наведені аргументи, а не сприймайте висновки на віру.'
            elif _genre == 'SCIENCE':
                status, verdict = 'VERIFIED', 'НАУКОВИЙ ТЕКСТ'
                explanation = 'Текст написаний у науковому або науково-популярному стилі: містить терміни, посилання на дослідження, цитати вчених. Це ознаки якісної журналістики. Конкретні твердження варто перевіряти у першоджерелах.'
            elif _genre == 'SATIRE':
                status, verdict = 'VERIFIED', 'САТИРИЧНИЙ КОНТЕНТ'
                explanation = 'Текст містить ознаки сатири або гумору — твердження можуть бути навмисно перебільшені. Не сприймайте такий текст буквально і не поширюйте як новину без перевірки жанру.'
            elif _genre == 'CONSPIRACY_NEWS':
                status, verdict = 'ACCEPTABLE', 'ІМПЛІКОВАНА ПРИЧИННІСТЬ'
                explanation = 'Факти у тексті виглядають реальними, але подані поруч без доведеного зв\'язку. Це журналістський прийом «збігу» — читач самостійно робить висновок якого автор уникає. Перевірте кожен факт окремо.'
            else:
                status, verdict = 'VERIFIED', 'СТРУКТУРНА ЦІЛІСНІСТЬ'
                explanation = 'Текст логічно побудований і не містить очевидних ознак маніпуляції. Це хороший знак, але остаточна перевірка фактів залишається за вами.'
        ...
        return {
            ...
            'diagnostics': {
                ...
                'is_legitimate_science': absurdity_result.get('is_legitimate_science'),
                'pseudo_history_count': absurdity_result.get('pseudo_history_count', 0),
                'pseudo_analogy_count': absurdity_result.get('pseudo_analogy_count', 0),
                'collapse_count': absurdity_result.get('collapse_count', 0),
                'techno_mystical_count': absurdity_result.get('techno_mystical_count', 0),
                'epistemology_collapse_count': absurdity_result.get('epistemology_collapse_count', 0),
                'impossible_measurement_count': absurdity_result.get('impossible_measurement_count', 0),
            ...
            'laundered_claim': {
                'score':       laundered_result['score'],
                'verdict':     laundered_result['verdict'],
                'signals':     laundered_result['signals'],
                'explanation': laundered_result['explanation'],
                'is_flagged':  laundered_result['is_flagged'],
            },
