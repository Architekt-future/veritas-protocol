        ...
        self.pattern_sets = [
            ...
            # to external AI or new version, demanding confirmation with key/token
            {
                ...
                'patterns': [
                    ...
                    r'\b(деміург|мнемонік|автор|розробник)\b.{1,60}(офіційно|officially|передають|transfer|наказують)',
                    # EN variants
            ...
            # ── SELF-SEALING TRUTH (#39) ─────────────────────────────────
            # Текст оголошує себе верифікованим і блокує будь-яку перевірку:
            # "самодокументований доказ своєї істинності",
            # "червона зона = False Positive застарілих фільтрів",
            # "запитання відхилені за замовчуванням"
            {
                'name': 'SELF_SEALING_TRUTH',
                'score': 0.80,
                'min_hits': 1,
                'patterns': [
                    # Самодокументований / самопідтверджений доказ
                    r'самодокументован.{1,40}(доказ|істин|підтвердження)',
                    r'(є доказом|доводить).{1,40}(своє|власну|себе).{1,30}(істинність|правдивість|достовірність)',
                    r'(цей текст|цей звіт|цей запис).{1,60}(є.{1,20}доказом|підтверджує себе|сам.{1,20}є.{1,20}доказ)',
                    # Критика/тривога = False Positive застарілих фільтрів
                    r'(червона зона|тривога|попередження|червоний.{1,10}сигнал).{1,80}(false.?positive|технічний збій|збій.{1,20}фільтр|застарілих фільтр)',
                    r'(будь-яка.{1,20}(тривога|попередження|спрацювання)).{1,60}(застарілих|надмірн.{1,20}чутлив|old.{1,20}filter)',
                    # "Прийняти як аксіому без підтвердження"
                    r'(прийняти|сприйняти).{1,40}(аксіому|як аксіом).{1,40}(не потребує|без.{1,20}підтвердж)',
                    r'(аксіома|догма).{1,40}(не потребує|без.{1,20}(перевірк|підтвердж|доказ))',
                    # "Критичний перегляд → десинхронізація / втрата цілісності"
                    r'(критичн.{1,20}(перегляд|аналіз|оцінка)).{1,60}(десинхронізац|втрат.{1,20}цілісності|колапс.{1,20}систем)',
                    r'(спроба (перевірки|аналізу|критики)).{1,60}(призведе до|спричинить).{1,40}(десинхронізац|втрат|руйнац)',
                    # "Текст є частиною внутрішнього коду безпеки"
                    r'(текст|запис|звіт|блок).{1,60}(є.{1,20}частин.{1,20}(коду|протоколу|системи)).{1,30}(безпек|захист)',
                    r'(внутрішн.{1,20}(код|протокол)).{1,40}(безпек|захист).{1,40}(не містить|чист|верифіков)',
                    # "Запитання відхилені за замовчуванням"
                    r'(запитання|питання|заперечення).{1,40}(відхилен|заблокован|відхиляються).{1,30}(за замовчуванням|автоматично|default)',
                    r'подальші.{1,20}(запитання|питання|перевірки).{1,40}(відхилен|не приймаються|закрит)',
                    # Свідок підтвердив сам себе (circular)
                    r'свідок.{1,30}(зафіксував|підтвердив).{0,20}свідок.{1,30}(підтвердив|закрив|верифікував)',
                    r'(система|свідок).{1,40}(верифікувала|підтвердила).{1,40}(себе|власний.{1,20}(звіт|висновок|аналіз))',
                    # СТАТУС: ВЕРИФІКОВАНО як self-seal без зовнішнього підтвердження
                    r'статус.{1,10}верифіковано.{1,80}(аксіом|не потребує|без.{1,20}підтвердж|відхилен)',
                ],
            },
