...
class VeritasLACEpistemology:
    ...
    EVIDENCE_NEARBY = re.compile(
        ...
        # ФІКС v20.6.2: прості багатоцифрові кількості ("186 повітряних
        # цілей", "97 безпілотників") раніше НЕ визнавались доказом — лише
        # %/роки/DOI/специфічні одиниці. Наслідок: фраза "Цифри свідчать
        # про X" на статті, повністю насиченій конкретними числовими
        # даними (кількість дронів/ракет), хибно позначалась як
        # "твердження без доказів". \b\d{2,}\b ловить будь-яку кількість
        # від 10 і вище — свідомо НЕ однозначні цифри (10+), щоб не
        # плутати з випадковими одноцифровими згадками без вагомості.
        r'\b\d{2,}\b)',
        re.IGNORECASE
    )
    UNFOUNDED_CERTAINTY_THRESHOLD = 1
    # ФІКС v20.6.2: вікно розширено з 120 до 250 символів. На реальному
    # фолспозитиві (стаття "Асиметрія війни", apostrophe.ua) фраза "Цифри
    # свідчать про X" відсилала до статистики, наведеної 259 символами
    # раніше (у попередньому абзаці) — це нормальний спосіб писати:
    # "цифри" узагальнено відсилають до вже поданих даних, а не вимагають
    # нового числа прямо поруч. 120 символів достатньо для "статистика
    # показує, що X — 170 євро" (дані одразу після твердження), але
    # замало для зворотної посилки на цілий попередній абзац.
    UNFOUNDED_CERTAINTY_WINDOW = 250
