...
class VeritasCalibratedCore:
    ...
    def analyze(self, text: str) -> Dict:
        ...
        # Analytics/multi-source text has naturally higher entropy — don't punish it
        import re as _genre_re
        _HEDGE = [r'\bвважають\b', r'\bне виключають\b', r'\bза оцінками\b',
                  r'\bаналітики\b', r'\bексперти\b', r'\bза словами\b',
                  r'\bзазначають\b', r'\bрозглядаються\b', r'\bімовірно\b',
                  r'\bнібито\b', r'\bповідомляється\b', r'\bочікується\b',
                  r'\bconsider\b', r'\banalysts\b', r'\baccording to\b',
                  r'\bexperts say\b', r'\bbelieve\b', r'\bappear to\b']
        _hedge_hits = sum(1 for p in _HEDGE if _genre_re.search(p, text, _genre_re.I))
        _named = _genre_re.findall(r'[А-ЯІЇЄ][а-яіїє]+(?:\s[А-ЯІЇЄ][а-яіїє]+)+', text)
        _unique_actors = len(set(_named))
        _genre = 'ANALYTICS' if _hedge_hits >= 2 else 'REPORT'  # 2+ hedge signals = analytics
        print(f'🎭 GENRE: hits={_hedge_hits}, genre={_genre}')
        # Analytics: raise entropy ceiling — high entropy is expected, not suspicious
        _genre_entropy_cap = 0.85 if _genre == 'ANALYTICS' else 1.0
        ...
        # AXIOM GUARD OVERRIDE
            ...
            if _genre == 'ANALYTICS':
                ...
                explanation = 'Текст демонструє ознаки аналітичного жанру: множинні джерела, хеджовані твердження, структурована аргументація'
            else:
