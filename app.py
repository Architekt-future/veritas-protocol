        ...
        if url and not text:
            try:
                ...
                SIDEBAR_BOUNDARY_MARKERS = [
                    ...
                    # v20.6: 'Поділитися' — кнопка "share", після якої на
                    # багатьох сайтах (напр. intent.press) йде блок тизерів
                    # інших новин, а іноді й ціла наступна стаття, вклеєна
                    # скрапером як продовження. 'Детальніше' свідомо НЕ додаю —
                    # це надто загальне слово, яке трапляється і в звичайній
                    # прозі ("детальніше про це нижче"), ризик хибного обрізання
                    # легітимного контенту переважує користь.
                    'Поділитися',
                ]
                # ФІКС v20.6: раніше цикл зупинявся на ПЕРШОМУУ ЗА ПОРЯДКОМ
                ...
                _sidebar_candidates = [
                    _idx for _m in SIDEBAR_BOUNDARY_MARKERS
