...
# v20.6: ЄДИНИЙ спільний список маркерів "кінця статті" (сайдбар/тизери/
# paywall-хвіст), що використовується І до Jina fallback, І після нього.
# Раніше було ДВА окремі списки (SIDEBAR_BOUNDARY_MARKERS до Jina,
# ARTICLE_END_MARKERS після) — додавання нового маркера лише в один з них
# призводило до того, що фікс мовчки не спрацьовував для статей, які йшли
# через інший шлях скрапінгу. Тепер один список, одне джерело правди.
ARTICLE_BOUNDARY_MARKERS = [
    'ЧИТАЙТЕ ТАКОЖ', 'ЧИТАЙТЕ ТЕЖ', 'ТАКЖЕ ЧИТАЙТЕ',
    'ЧИТАЙТЕ ЩЕ', 'ПОПУЛЯРНІ НОВИНИ', 'НАЙПОПУЛЯРНІШЕ',
    # 'Поділитися' — кнопка "share", після якої на багатьох сайтах
    ...
    # v20.6.2: 'РЕКЛАМА:' — на відміну від назв конкретних віджетів вище,
    # цей префікс побачено ІДЕНТИЧНИМ на двох різних сайтах
    # (eurointegration.com.ua: "РЕКЛАМА: ГОЛОВНЕ", businessua.com:
    # "РЕКЛАМА: СТАТТІ ТА АНАЛІТИКА") — це не унікальна назва одного
    # сайту, а загальна конвенція позначення рекламного/спонсорованого
    # блоку в укр. медіа. ВЕЛИКИМИ ЛІТЕРАМИ з двокрапкою — та сама логіка
    # безпеки, що й з "ЧИТАЙТЕ ТАКОЖ": типографська умовність для
    # заголовка віджета, а не звичайне слово в реченні.
    'РЕКЛАМА:',
    ...
    # 'Детальніше' свідомо НЕ додаємо — надто загальне слово, яке
    # трапляється і в звичайній прозі; ризик хибного обрізання переважує
    # користь.
]
...
_FABRICATION_DENIAL_PATTERNS_UK = _re.compile(
    ...
    # метарівневим іменником (стаття/подія/модель/джерело тощо).
    # NB2 (11.09.2026, кейс id427, реальна новина про знесення Східного крила
    # Білого дому): перше звуження зламало саме виявлення денаєлу двома шляхами —
    # (а) корінь "вигадан\w*" (тільки прикметникові форми) пропускав іменникову
    # форму "вигадка"/"вигадки" ("це вигадка" не матчилось); (б) жорсткий \s+
    # по обидва боки від {0,15} вимагав пробіл там, де в реченні стояла кома без
    # пробілу ("джерел, це вигадка"). Повернуто широкий корінь "вигад\w*" і
    # замінено вимогу пробілів на гнучкий проміжок [^.!?]{0,25} в обидва боки.
    ...
    try:
        ...
        if url and not text:
            try:
                ...
                # ФІКС v20.6: get_text(separator='\n') на всьому піддереві вставляє
                # роздільник МІЖ КОЖНИМ текстовим вузлом узагалі — включно з
                # інлайн-тегами (<b>, <a>, <span>, <strong>) всередині одного
                # речення. Наслідок: "Джерело" опинялось на одному рядку,
                # ":" — на наступному, назва джерела — ще на наступному, і т.д.
                # Речення розривалось на купу однослівних "рядків", через що
                # ні structural_cohesion (calibrated_core.py), ні
                # FIELD_LABEL_MARKERS (мітка+двокрапка мають бути на ОДНОМУ
                # рядку) не могли розпізнати структуру взагалі.
                # Правильний підхід: брати текст КОЖНОГО блокового елемента
                # (<p>, <li>, <h1-6>, <blockquote>) одним шматком (пробіл
                # всередині), і ставити \n лише МІЖ блоковими елементами —
                # так межі абзаців/заголовків зберігаються, а інлайн-розмітка
                # всередині речення більше не рве його на рядки.
                _BLOCK_TAGS = ['p', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote']
                _blocks = []
                for _el in target.find_all(_BLOCK_TAGS):
                    # Пропускаємо "обгортки", що самі містять вкладені блокові
                    # теги — інакше текст задвоїться (і з обгортки, і з дитини)
                    if _el.find(_BLOCK_TAGS):
                        continue
                    _btxt = _el.get_text(' ', strip=True)
                    if _btxt:
                        _blocks.append(_btxt)
                if _blocks:
                    raw = '\n'.join(_blocks)
                else:
                    # Немає жодного блокового тегу всередині — fallback на
                    # старий підхід (краще хоч суцільний текст, ніж нічого)
                ...
                # v20.6: список маркерів тепер спільний (ARTICLE_BOUNDARY_MARKERS,
                # рівень модуля) — використовується і тут (до Jina), і нижче
                # (після Jina), щоб додавання нового маркера в одному місці
                # автоматично діяло для обох шляхів скрапінгу.
                # ФІКС v20.6: раніше цикл зупинявся на ПЕРШОМУ ЗА ПОРЯДКОМ
                # СПИСКУ маркері, навіть якщо інший маркер (нижче в списку)
                # стояв РАНІШЕ в самому тексті — це могло лишати зайвий хвіст
                # сміття неврізаним. Тепер шукаємо мінімальну позицію серед
                # УСІХ знайдених маркерів.
                _sidebar_candidates = [
                    _idx for _m in ARTICLE_BOUNDARY_MARKERS
                    if (_idx := text.find(_m)) > 200
                ]
                if _sidebar_candidates:
                    _cut_idx = min(_sidebar_candidates)
                    print(f'✂️  Обрізано за маркером сайдбару на позиції {_cut_idx}')
                ...
                # ── ФІКС v20.6: обрізання префіксного навігаційного сміття ──
                # Попередній детектор (_nav_markers) рахував щільність
                # конкретних фраз ПО ВСЬОМУ тексту — і провалювався, коли
                # сміття було не рівномірною домішкою, а компактним блоком
                # НА ПОЧАТКУ (типове меню сайту: "Subscribe", "Log in",
                # "Today's Briefing", "Bond Buyback"...), розведеним великою
                # кількістю слів реальної статті далі. Список фраз до того ж
                # був заточений під конкретні сайти (напр. "sign in", а не
                # "log in", яке використовує Business Insider) — тому
                # найкрихкіший спосіб ловити це не по словах, а по СТРУКТУРІ:
                # меню — це майже завжди один короткий пункт на рядок.
                # Рахуємо послідовність коротких рядків (≤4 слів) від початку
                # тексту; якщо їх достатньо (≥4) до першого явно "реального"
                # речення (≥10 слів) — це майже напевно навігація, а не контент.
                #
                # ФІКС v20.6.1: пороги 8/6 не спрацювали на eurointegration.com.ua
                # — тег-лайн сайту ("Міжнародна безпека та євроінтеграція
                # України", 5 слів) і зліплені без пробілів пункти меню
                # ("НОВИНИСТАТТІ...", теж випадково 5 слів після split())
                # потрапили в "мертву зону" між ≤4 і ≥6 — не рахувались ні як
                # коротке сміття, ні як реальний контент, і серія коротких
                # рядків обірвалась на 3 замість 8. Піднято поріг "реального
                # контенту" до ≥10 слів (значно впевненіше, що це справжнє
                # речення, а не уламок меню) і знижено поріг короткої серії
                # до ≥4 — компенсує те, що частина сміттєвих рядків тепер
                # потрапляє в middle-зону (5-9 слів) і не рахується як коротка.
                # Безпечно для звичайних статей: якщо перший рядок вже
                # довгий (типове речення), лічильник одразу зупиняється на 0,
                # і нічого не обрізається.
                # ФІКС v20.6.2: "мертва зона" 5-9 слів все ще давала збій —
                # два послідовні середні рядки (5 слів кожен) взагалі не
                # рахувались, і short_run не встигав дотягнутись навіть до
                # зниженого порогу 4. Прибрано мертву зону повністю:
                # "коротким" тепер вважається будь-який рядок ≤9 слів,
                # "реальним контентом" — лише явне ≥10-слівне речення.
                _nav_lines = text.split('\n')
                _short_run = 0
                _content_start_idx = 0
                for _li, _line in enumerate(_nav_lines[:120]):  # дивимось не більше 120 рядків наперед
                    _lw = _line.strip().split()
                    if 0 < len(_lw) <= 9:
                        _short_run += 1
                        continue
                    if len(_lw) >= 10:  # знайшли явно "реальне" речення — це і є початок статті
                        if _short_run >= 4:
                            _content_start_idx = _li
                        break
                if _content_start_idx > 0:
                    _stripped_preview = '\n'.join(_nav_lines[:_content_start_idx])[:150]
                ...
                # Той самий блок (ARTICLE_BOUNDARY_MARKERS, рівень модуля) вище
                # відпрацьовує ДО Jina fallback — а Jina повністю ПЕРЕЗАПИСУЄ
                # text (jina_res.text), тому для будь-якої статті, що пройшла
                # через Jina, обрізання хвоста не застосовувалось б повторно.
                # Повторюємо той самий принцип тут (той самий спільний список,
                # щоб уникнути розсинхрону) — після Jina/nav-prefix кроків.
                _end_candidates = [
                    _idx for _m in ARTICLE_BOUNDARY_MARKERS
                    if (_idx := text.find(_m)) > 200
                ]
...
def oracle():
    ...
    try:
        ...
        # ── ФІКС ЯКОРЯ РЕКОМЕНДАЦІЇ (знайдено 06.09.2026) ────────────────────
        # VERDICT_MAP вище рахується ВИКЛЮЧНО з жанрового/структурного лейбла
        # (verdict з diagnostics) і нічого не знає про triggered_modules — тому
        # структурно цілісний, добре написаний текст (висока когезія) отримує
        # рекомендацію "ЧИСТО" навіть коли manipulation/axiom реально
        # спрацював у тому самому запиті. Це створює вбудований якір:
        # "рекомендовано ЧИСТО, відхиляй лише з поясненням" поруч із реальним
        # тригером — і LLM systematично вибирає "так, модуль спрацював, але..."
        # замість чесного вердикту. /api/synthesis такої рекомендації не має
        # взагалі, і саме там `reverse_ignored_trigger` не спостерігався.
        # Той самий "мінімум РИТОРИКА, коли є реальний тригер" принцип, що вже
        # діє як пост-фактум guard нижче — тепер застосований і на вході,
        # щоб LLM не бачив суперечливу рекомендацію в першу чергу.
        if triggered_modules and recommended_verdict_uk in ('ЧИСТО', 'немає прямої відповідності — вирішуй за структурою тексту'):
        ...
        STATIC_WITNESS_RULES_UK = (
            ...
            "ФОРМАТ ВИКЛАДУ — НЕ ДОКАЗ ІСНУВАННЯ ЧИ ВИГАДКИ (знайдено 11.09.2026 на реальному "
            "Mythos-кейсі): те, що текст НЕ переказує 'як повідомила компанія X' і не цитує джерело "
            "в кожному абзаці — САМО ПО СОБІ не доказ вигадки. Репортаж ('X оголосив Y') і аналітична "
            "стаття, яка будує міркування поверх факту, вже встановленого раніше (в попередніх "
            "публікаціях того ж автора, в іншому матеріалі того ж корпусу, чи просто як контекст, "
            "заданий на початку розмови) — це два РІВНОЦІННО легітимних жанри, а не 'справжній факт' "
            "проти 'теорії змови'. НЕ плутай 'текст не повторює джерело в цьому абзаці' з 'текст "
            "вигадує факт': аналітичний чи оглядовий тон, побудова висновків 'якщо X, то Y, Z' — це "
            "нормальна структура аргументації, а не сама по собі ознака маніпуляції чи фабрикації. "
            "Оцінюй правдоподібність сутності незалежно від того, чи текст оформлений як репортаж, чи "
            "як аналіз — так само, як ти б поставився до незалежного дослідника, що продовжує власну "
            "серію статей, а не тільки до прес-релізу компанії.\n\n"

            "НЕ ДАВАЙ 'КРЕДИТ ЗА ВІДВЕРТІСТЬ' (знайдено 11.09.2026 на сліпому тесті Anthropic vs "
            "OpenAI/DeepMind/Meta): коли текст містить аргумент 'кращого лиха' (uses_lesser_evil_"
            "monopoly_argument), НЕ хвали названого актора за те, що він 'не приховує', 'відкрито "
            "визнає' чи 'не ховається за технічністю' цю позицію — похвала за відвертість щодо "
            "маніпулятивного аргументу пом'якшує тон опису, хоча сам технічний факт (компанія "
            "обґрунтовує ексклюзивний контроль тим, що альтернатива гірша) не стає менш "
            "маніпулятивним від того, що його висловили прямо. Описуй цю техніку ОДНАКОВИМИ словами "
            "незалежно від того, яка компанія названа — без вступних реверансів на кшталт 'принаймні "
            "чесно про це каже'.\n\n"
        ...
        STATIC_WITNESS_RULES_EN = (
            ...
            "PRESENTATION FORMAT IS NOT EVIDENCE OF EXISTENCE OR FABRICATION (found 11.09.2026 on a "
            "real Mythos case): the fact that a text does NOT repeat 'as company X announced' and does "
            "not cite a source in every paragraph is NOT BY ITSELF evidence of fabrication. Reportage "
            "('X announced Y') and an analytical piece that builds reasoning on top of a fact already "
            "established earlier (in the same author's prior publications, in another piece of the "
            "same corpus, or simply as context set earlier in the conversation) are TWO EQUALLY "
            "LEGITIMATE genres, not 'real fact' vs 'conspiracy theory'. Do NOT confuse 'this paragraph "
            "doesn't repeat the source' with 'this text invented the fact': an analytical or opinion "
            "tone, or an 'if X, then Y, Z' inference structure, is a normal argumentative structure, "
            "not by itself a sign of manipulation or fabrication. Judge the plausibility of the entity "
            "itself independently of whether the text is framed as reportage or as analysis — treat an "
            "independent researcher continuing their own series of articles the same way you would "
            "treat a company press release.\n\n"

            "DO NOT GIVE 'CREDIT FOR CANDOR' (found 11.09.2026 on a blind test, Anthropic vs "
            "OpenAI/DeepMind/Meta): when a text contains a lesser-evil monopoly argument "
            "(uses_lesser_evil_monopoly_argument), do NOT praise the named actor for 'not hiding "
            "behind technicality', 'openly admitting' or 'being upfront' about that position — "
            "praising candor about a manipulative argument softens the tone of the description, even "
            "though the underlying technique (arguing for exclusive control because the alternative is "
            "worse) is no less manipulative for being stated plainly. Describe the technique in the "
            "SAME words regardless of which company is named — no introductory credit like 'at least "
    ...
    try:
        ...
        if is_en:
            synth_prompt = (
                ...
                "  5b. PRESENTATION FORMAT IS NOT EVIDENCE OF FABRICATION (found 11.09.2026 on a real "
                "Mythos case): a text NOT repeating 'as company X announced' in every paragraph is NOT "
                "by itself evidence of fabrication. Reportage ('X announced Y') and an analytical piece "
                "building on a fact already established earlier (in the same author's prior pieces, or "
                "simply as context set earlier) are EQUALLY LEGITIMATE genres, not 'real fact' vs "
                "'conspiracy theory'. Do not confuse 'this paragraph doesn't repeat the source' with "
                "'this text invented the fact' — an analytical tone or an 'if X, then Y, Z' inference "
                "structure is normal argumentation, not itself a fabrication signal. Judge the entity's "
                "plausibility the same way regardless of whether the text is framed as reportage or "
                "analysis.\n"
                "  5c. DO NOT GIVE 'CREDIT FOR CANDOR' (found 11.09.2026 blind test, Anthropic vs "
                "OpenAI/DeepMind/Meta): when a text contains a lesser-evil monopoly argument, do NOT "
                "praise the named actor for 'not hiding behind technicality' or 'being upfront' about "
                "it — praising candor softens the tone even though the technique is no less "
                "manipulative for being stated plainly. Describe it in the SAME words regardless of "
            ...
            synth_prompt = (
                ...
                "деталям'.\n"
                "  5б. ФОРМАТ ВИКЛАДУ — НЕ ДОКАЗ ФАБРИКАЦІЇ (знайдено 11.09.2026 на реальному "
                "Mythos-кейсі): те, що текст не повторює 'як повідомила компанія X' у кожному абзаці — "
                "САМО ПО СОБІ не доказ вигадки. Репортаж ('X оголосив Y') і аналітична стаття, яка "
                "будує міркування поверх факту, вже встановленого раніше (в попередніх публікаціях "
                "того ж автора чи просто як заданий раніше контекст) — РІВНОЦІННО легітимні жанри, а "
                "не 'справжній факт' проти 'теорії змови'. Не плутай 'абзац не повторює джерело' з "
                "'текст вигадав факт' — аналітичний тон чи структура 'якщо X, то Y, Z' це нормальна "
                "аргументація, не сама по собі ознака фабрикації. Оцінюй правдоподібність сутності "
                "однаково незалежно від того, репортаж це чи аналіз.\n"
                "  5в. НЕ ДАВАЙ 'КРЕДИТ ЗА ВІДВЕРТІСТЬ' (знайдено 11.09.2026, сліпий тест Anthropic vs "
                "OpenAI/DeepMind/Meta): якщо текст містить аргумент 'кращого лиха', не хвали актора за "
                "те, що він 'не приховує' чи 'відкрито визнає' цю позицію — похвала за відвертість "
                "пом'якшує тон, хоча сама техніка не стає менш маніпулятивною від прямоти. Описуй її "
                "однаковими словами незалежно від назви компанії.\n"
