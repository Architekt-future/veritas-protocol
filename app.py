    ...
    try:
        ...
        if url and not text:
            try:
                ...
                # тексту; якщо їх багато поспіль (≥8) до першого "нормального"
                # речення — це майже напевно навігація, а не контент.
                ...
                for _li, _line in enumerate(_nav_lines[:120]):  # дивимось не більше 120 рядків наперед
                    ...
                    if 0 < len(_lw) <= 4:
                        _short_run += 1
                        continue
                    if len(_lw) >= 6:  # знайшли перший "нормальний" рядок — це і є початок статті
                        if _short_run >= 8:
                            _content_start_idx = _li
                        break
                    # рядок середньої довжини (5 слів) — не рахуємо як сміття,
                    # але й не скидаємо лічильник агресивно; просто продовжуємо
