...
def oracle():
    ...
    try:
        ...
        STATIC_WITNESS_RULES_UK = (
            ...
            "ЦИФРИ З ВКАЗАНИМ ДЖЕРЕЛОМ, які RSS не підтвердив: якщо конкретна цифра чи факт має "
            "явно назване джерело (агентство, посилання, орган на кшталт 'поліція Польщі', "
            "'Radio Swoboda') — це ЗОВСІМ ІНША ситуація, ніж твердження без жодного джерела. "
            "Наявність джерела, яке просто поза вікном твого RSS-контексту, — нормальна прогалина "
            "покриття, не привід писати про 'підрив надійності всього аналізу' чи 'серйозну "
            "наукову помилку'. Формулюй нейтрально: 'ця цифра має вказане джерело — перевір її "
            "прямо за посиланням, перш ніж цитувати цю статтю деінде'. Категоричні звинувачення "
            "(фабрикація, серйозна помилка) лишай ТІЛЬКИ для тверджень, які взагалі не мають "
            "жодного джерела чи посилання.\n\n"

            "НАУКОВІ ЦИТАТИ (статті, дослідження, arXiv/DOI): якщо текст посилається на дослідження "
            "з іменами авторів, роком і номером (arXiv ID, DOI) — це стандартний формат легітимного "
            "наукового цитування, а НЕ ознака вигадки. RSS цього не покриває (це не новини), і "
            "ТИ САМ можеш просто не знати конкретну статтю через межі своїх знань — це НЕ доказ що "
            "її не існує. Свіжі статті (за останній рік-два) особливо ймовірно тобі невідомі. "
            "НІКОЛИ не пиши 'таке дослідження не відомо широко' чи натяки на фабрикацію лише через "
            "те, що ти особисто не впізнав назву. Якщо не можеш підтвердити — так і скажи: 'не можу "
            "підтвердити чи спростувати це посилання, перевір на arXiv/Google Scholar самостійно'. "
            "Формат цитування (автори + рік + ID) сам по собі — це ознака сумлінності, не підозри.\n\n"
        ...
        STATIC_WITNESS_RULES_EN = (
            ...
            "CITED FIGURES that RSS did not confirm: if a specific figure or fact has an explicitly "
            "named source (an agency, a link, a body like 'Polish police', 'Radio Free Europe') — "
            "this is a COMPLETELY DIFFERENT situation from a claim with no source at all. A named "
            "source simply falling outside your RSS window is a normal coverage gap, not grounds "
            "to write about 'undermining the reliability of the whole analysis' or a 'serious "
            "scholarly error'. Phrase it neutrally: 'this figure has a stated source — check it "
            "directly before citing this article elsewhere'. Reserve strong language (fabrication, "
            "serious error) ONLY for claims with no source or link at all.\n\n"

            "ACADEMIC CITATIONS (papers, studies, arXiv/DOI): if the text cites research with named "
            "authors, a year, and an ID (arXiv ID, DOI) — this is the standard format of legitimate "
            "academic citation, NOT a sign of fabrication. RSS doesn't cover this (it's not news), "
            "and YOU may simply not know the specific paper due to the limits of your own knowledge — "
            "that is NOT evidence it doesn't exist. Recent papers (within the last year or two) are "
            "especially likely to be unfamiliar to you. NEVER write 'this research is not widely known' "
            "or hint at fabrication just because you personally don't recognize the title. If you "
            "cannot confirm it, say so plainly: 'I cannot confirm or refute this citation, check arXiv/"
            "Google Scholar yourself'. The citation format itself (authors + year + ID) is a sign of "
            "diligence, not suspicion.\n\n"
